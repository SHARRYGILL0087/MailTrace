        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <div className={`flex h-8 w-8 items-center justify-center rounded-xl ${
              isConnected ? 'bg-blue-50 text-blue-700' : 'bg-rose-50 text-rose-700'
            }`}>
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${
              ...
              <span className={`h-1.5 w-1.5 rounded-full ${isConnected ? 'bg-blue-600' : 'bg-rose-600'}`} />
              {result.status}
            </span>
          </div>
          ...
          {result.latencyMs && (
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono font-bold text-blue-700">{result.latencyMs} ms</span>
            </div>
          )}
